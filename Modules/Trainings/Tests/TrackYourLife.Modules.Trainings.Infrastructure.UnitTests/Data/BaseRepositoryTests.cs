...
namespace TrackYourLife.Modules.Trainings.Infrastructure.UnitTests.Data;
...
public class DatabaseFixture : IAsyncLifetime
{
    ...
    public TrainingsWriteDbContext WriteDbContext { get; private set; } = null!;
    public TrainingsReadDbContext ReadDbContext { get; private set; } = null!;
    ...
    public async Task InitializeAsync()
    {
        ...
        var writeOptions = new DbContextOptionsBuilder<TrainingsWriteDbContext>()
            .UseNpgsql(DbContainer.GetConnectionString())
            .Options;

        var readOptions = new DbContextOptionsBuilder<TrainingsReadDbContext>()
            .UseNpgsql(DbContainer.GetConnectionString())
            .Options;

        WriteDbContext = new TrainingsWriteDbContext(writeOptions, configuration);
        ReadDbContext = new TrainingsReadDbContext(readOptions, configuration);
...
public abstract class BaseRepositoryTests : IAsyncLifetime
{
    ...
    protected TrainingsWriteDbContext WriteDbContext => _fixture.WriteDbContext;
    protected TrainingsReadDbContext ReadDbContext => _fixture.ReadDbContext;
