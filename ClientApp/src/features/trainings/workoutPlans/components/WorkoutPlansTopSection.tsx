...
function WorkoutPlansTopSection() {
  ...
  const nextWorkoutIndexInPlan = nextWorkoutId
    ? activePlan.workouts.findIndex((w) => w.id === nextWorkoutId)
    : -1;
  const safeNextWorkoutIndexInPlan = Math.max(nextWorkoutIndexInPlan, 0);

  const completedCount = safeNextWorkoutIndexInPlan;
  const progressPercent =
    activePlan.workouts.length === 0
      ? 0
      : Math.round((completedCount / activePlan.workouts.length) * 100);
          ...
          <div className="flex-1">
            <div className="mb-2 flex items-center justify-between">
              ...
              <span className="text-sm font-medium text-primary">
                {completedCount}/{activePlan.workouts.length} workouts
              </span>
            </div>
