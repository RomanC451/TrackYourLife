...
import useCalculateNutritionGoalsMutation from "@/features/nutrition/goals/mutations/useCalculateNutritionGoalsMutation";
...
function CalculateNutritionGoalsForm({ onSuccess }: { onSuccess: () => void }) {
  ...
  const calculateNutritionGoalsMutation = useCalculateNutritionGoalsMutation();
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
        ...
        <ButtonWithLoading
          type="submit"
          className="w-full"
          isLoading={calculateNutritionGoalsMutation.isDelayedPending}
